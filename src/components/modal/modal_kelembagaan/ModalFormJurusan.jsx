...
export const ModalDetailJurusan = ({ isOpen, onClose, id }) => {
    console.log(id);
    ...
    useEffect(() => {
        if (isOpen && id) {
            const token = sessionStorage.getItem("token") || getCookie("token");
            setLoading(true);
            fetch(`${API_BASE_URL}crud/jurusan/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                },
                ...
                <div className="flex items-center justify-center min-h-screen px-4 py-8 text-center">
                    ...
                    >
                        <Dialog.Panel className="bg-white rounded-lg shadow-xl max-w-lg w-full h-full relative max-h-[90vh] flex flex-col">
                            ...
                            {/* Header */}
                            <div className="pt-6">
                                <Dialog.Title className="text-lg font-semibold text-gray-900">Detail Jurusan</Dialog.Title>
                            </div>

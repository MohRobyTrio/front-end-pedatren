...
export const ModalAddOrEditRoles = ({ isOpen, onClose, data, refetchData }) => {
    const { clearAuthData } = useLogout();
    const navigate = useNavigate();
    const [formData, setFormData] = useState({
        name: "",
        guard_name: "web", // Default value
        permissions: [], // Berupa array untuk menampung nama permission yang dipilih
    });
    ...
    useEffect(() => {
        const fetchPermissions = async () => {
            ...
            try {
                ...
                if (!response.ok) throw new Error("Gagal mengambil data permissions");
                const data = await response.json();
                setAvailablePermissions(data);
            } catch (error) {
                console.error(error);
                Swal.fire("Error", "Tidak dapat memuat daftar permission.", "error");
            } finally {
        ...
        if (isOpen) {
            fetchPermissions(); // Panggil fungsi fetch

            if (data) { // Mode Edit
                setFormData({
                    name: data.name || "",
                    guard_name: data.guard_name || "",
                    permissions: data.permissions ? data.permissions.map(p => p.name) : [],
                });
            } else { // Mode Tambah
                setFormData({
                    name: "",
    ...
    const handlePermissionChange = (permissionName) => {
        setFormData(prevFormData => {
            const currentPermissions = prevFormData.permissions;
            if (currentPermissions.includes(permissionName)) {
                // Jika sudah ada, hapus dari array
                return { ...prevFormData, permissions: currentPermissions.filter(p => p !== permissionName) };
            } else {
                // Jika belum ada, tambahkan ke array
                return { ...prevFormData, permissions: [...currentPermissions, permissionName] };
            }
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.permissions.length == 0) {
            await Swal.fire({ icon: "error", title: "Validasi Gagal", text: "Minimal pilih satu permission." });
            return;
        }
                            ...
                            <form className="w-full" onSubmit={handleSubmit}>
                                <div className="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                    <div className="sm:flex sm:items-start">
                                        <div className="mt-2 sm:mt-0 text-left w-full">
                                            ...
                                            <div className="space-y-4">
                                                <div>
                                                    <label htmlFor="name" className="block text-gray-700">Nama Role *</label>
                                                    <input
                                    ...
                                    {/* <div className="mt-4">
                                        ...
                                        <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-md p-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    ...
                                    <div className="mt-4">
                                        ...
                                        {loadingPermissions ? (
                                            ...
                                            <div className="border border-gray-300 rounded-md p-3 max-h-40 overflow-y-auto">{availablePermissions.length > 0 ? (
                                                <div className="space-y-2">{
                                                    availablePermissions.map((item) => (
                                                        <label key={item.id} className="flex items-center cursor-pointer">
                                                            <input type="checkbox" checked={formData.permissions.includes(item.name)} onChange={() => handlePermissionChange(item.name)} className="form-checkbox text-blue-600 rounded" />
                                                            <span className="ml-2 text-sm text-gray-700">{item.name}</span>
                                                        </label>
                                                    ))}
                                                </div>
                                            ) : (
                                                <p className="text-gray-500 text-sm">Tidak ada permissions tersedia</p>
                                            )}
                                            </div>
                                        )}
                                        {formData.permissions.length == 0 && !loadingPermissions && <p className="text-red-500 text-xs mt-1">Minimal pilih satu permissions</p>}
                                    </div>
                                </div>
...
export const ModalDetailRoles = ({ isOpen, onClose, id }) => {
                ...
                <div className="flex items-center justify-center min-h-screen px-4 py-8 text-center">
                    ...
                    >
                        <Dialog.Panel className="bg-white rounded-lg shadow-xl max-w-lg w-full h-full relative max-h-[90vh] flex flex-col">
                            ...
                            <div className="pt-6">
                                <Dialog.Title className="text-lg font-semibold text-gray-900">Detail Roles</Dialog.Title>
                            </div>

...
export const ModalAddOrEditAngkatan = ({ isOpen, onClose, data, refetchData }) => {
        ...
        if (isOpen) {
            if (data) {
                setFormData({
                    ...
                    angkatan: data.angkatan.match(/\d{4}/)?.[0],
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const isEdit = data;
            const url = isEdit
                ? `${API_BASE_URL}crud/angkatan/${data.id}`
                : `${API_BASE_URL}crud/angkatan`;
                                    ...
                                    <div className="sm:flex sm:items-start">
                                        <div className="mt-2 sm:mt-0 text-left w-full">
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div>
                                                    <label htmlFor="angkatan" className="block text-gray-700">Angkatan *</label>
                                                    <input
                                                        type="text"
                                                        maxLength={4}
...
export const ModalDetailAngkatan = ({ isOpen, onClose, id }) => {
    console.log(id);
    ...
    useEffect(() => {
        if (isOpen && id) {
            const token = sessionStorage.getItem("token") || getCookie("token");
            setLoading(true);
            fetch(`${API_BASE_URL}data-pokok/angkatan/${id}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                },
                ...
                <div className="flex items-center justify-center min-h-screen px-4 py-8 text-center">
                    ...
                    >
                        <Dialog.Panel className="bg-white rounded-lg shadow-xl max-w-lg w-full h-full relative max-h-[90vh] flex flex-col">
                            ...
                            {/* Header */}
                            <div className="pt-6">
                                <Dialog.Title className="text-lg font-semibold text-gray-900">Detail Angkatan</Dialog.Title>
                            </div>

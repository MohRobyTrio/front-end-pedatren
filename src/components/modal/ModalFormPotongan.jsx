...
export const ModalAddOrEditPotongan = ({ isOpen, onClose, data, refetchData, feature }) => {
    ...
    const [formData, setFormData] = useState({
        nama: "",
        kategori: 'umum', // Field Kategori ditambahkan
        jenis: 'persentase',
        nilai: "",
        status: true,
        keterangan: "",
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (feature === 2 && data) {
                setFormData({
                    nama: data.nama || "",
                    kategori: data.kategori || "umum", // Set kategori saat edit
                ...
                setFormData({
                    nama: "",
                    kategori: 'umum', // Reset kategori untuk data baru
                    jenis: 'persentase',
                    nilai: "",
                    status: true,
                    keterangan: "",
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.tagihan_ids.length === 0) {
            await Swal.fire({
                ...
                text: "Minimal pilih satu kategori tagihan.",
            });
            return;
        }
        ...
        try {
            ...
            // validasi gagal (Laravel FormRequest)
                        ...
                        <Dialog.Panel className="inline-block overflow-y-auto align-bottom bg-white rounded-lg text-left shadow-xl transform transition-all w-full max-w-md sm:max-w-2xl sm:align-middle">
                            ...
                            <form className="w-full" onSubmit={handleSubmit}>
                                    ...
                                    <div className="sm:flex sm:items-start">
                                        <div className="mt-2 sm:mt-0 text-left w-full">
                                            ...
                                            <div className="space-y-4">
                                                {/* Nama Potongan Field (Input Teks) */}
                                                <div>
                                                ...
                                                {/* Kategori Field (Dropdown Baru) */}
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                                        Kategori <span className="text-error-500">*</span>
                                                    </label>
                                                    <Listbox value={formData.kategori} onChange={(value) => setFormData(prev => ({ ...prev, kategori: value }))}>
                                                            ...
                                                            <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                                                                <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-lg bg-white py-1 shadow-lg ring-1 ring-black/5 focus:outline-none">
                                                                    {KATEGORI_OPTIONS.map((option) => (
                                                                        <Listbox.Option key={option.value} className={({ active }) => `relative cursor-pointer select-none py-2 pl-10 pr-4 ${active ? 'bg-primary-100 text-primary-900' : 'text-gray-900'}`} value={option.value}>
                                                                            {({ selected }) => (
                                                                                <>
                                                                                    <span className={`block truncate ${selected ? 'font-medium' : 'font-normal'}`}>{option.label}</span>
                                                                                    {selected && (<span className="absolute inset-y-0 left-0 flex items-center pl-3 text-primary-600"><FontAwesomeIcon icon={faCheck} className="h-5 w-5" /></span>)}
                                                                                </>
                                                                            )}
                                                                        </Listbox.Option>
                                                                    ))}
                                                ...
                                                <div>
                                                    ...
                                                    <div className="relative">
                                                        <input
                                                            ...
                                                            value={formData.jenis === "nominal" ? (formData.nilai ? new Intl.NumberFormat("id-ID").format(formData.nilai) : "") : formData.nilai}
                                                            onChange={(e) => {
                                                            ...
                                                            className={`w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 ${formData.jenis === "nominal" ? "pl-10" : "pr-8"}`}
                                                            placeholder={formData.jenis === "persentase" ? "0-100" : "0"}
                                                            required
                                                        />
                                                        {formData.jenis === 'nominal' && (<div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none"><span className="text-gray-500 text-sm">Rp</span></div>)}
                                                        {formData.jenis === 'persentase' && (<div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none"><span className="text-gray-500 text-sm">%</span></div>)}
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <textarea id="keterangan" value={formData.keterangan} onChange={(e) => setFormData(prev => ({ ...prev, keterangan: e.target.value }))} rows={3} className="w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 resize-none" placeholder="Masukkan keterangan (opsional)" />
                                                </div>

                                                {/* Berlaku Untuk Tagihan */}
                                                <div>
                                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                                        Berlaku Untuk Tagihan <span className="text-error-500">*</span>
                                                    </label>
                                                    {loadingTagihan ? (
                                                        <div className="flex justify-center py-4"><OrbitProgress variant="disc" color="#2a6999" size="small" /></div>
                                                    ) : (
                                                                ...
                                                                <div className="space-y-2">
                                                                    {tagihan.map((item) => (
                                                                        <label key={item.id} className="flex items-center">
                                                                            <input type="checkbox" checked={formData.tagihan_ids.includes(item.id)} onChange={() => handleCategoryChange(item.id)} className="form-checkbox text-blue-600 rounded" />
                                                ...
                                                <div>
                                                    <label className="flex items-center space-x-3">
                                                        <input type="checkbox" checked={formData.status} onChange={(e) => setFormData(prev => ({ ...prev, status: e.target.checked }))} className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded" />
                                ...
                                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                    <button type="submit" className="w-full cursor-pointer inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-500 text-base font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:w-auto sm:text-sm">Simpan</button>
                                    <button type="button" onClick={onClose} className="mt-3 cursor-pointer w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 sm:mt-0 sm:w-auto sm:text-sm">Batal</button>
                                </div>
                            </form>
                        </Dialog.Panel>
                    </Transition.Child>

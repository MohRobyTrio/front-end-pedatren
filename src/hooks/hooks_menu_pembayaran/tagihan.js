...
const useFetchTagihan = (filters) => {
    ...
    const fetchTagihan = useCallback(async () => {
        ...
        let url = `${API_BASE_URL}tagihan?page=${currentPage}`;

        if (debouncedSearchTerm) url += `&q=${encodeURIComponent(debouncedSearchTerm)}`;
        if (filters?.status) url += `&status=${encodeURIComponent(filters.status)}`;
        if (filters?.tipe) url += `&tipe=${encodeURIComponent(filters.tipe)}`;
        ...
        try {
            ...
            setTagihan(Array.isArray(result.data.data) ? result.data.data : []);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            const token = sessionStorage.getItem("token") || getCookie("token");
            const response = await fetch(`${API_BASE_URL}tagihan/${id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
...
export default useFetchTagihan;
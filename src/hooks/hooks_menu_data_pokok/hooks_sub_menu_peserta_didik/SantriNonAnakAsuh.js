...
const useFetchSantriNonAnakAsuh = (filters) => {
    const { clearAuthData } = useLogout();
    const navigate = useNavigate();
    const [santriNonAnakAsuh, setSantriNonAnakAsuh] = useState([]);
    const [loadingSantriNonAnakAsuh, setLoadingSantriNonAnakAsuh] = useState(true);
    const [error, setError] = useState(null);
    const [limit, setLimit] = useState(25);
    const [totalDataSantriNonAnakAsuh, setTotalDataSantriNonAnakAsuh] = useState(0);
    const [totalPages, setTotalPages] = useState(1);
    const [searchTerm, setSearchTerm] = useState("");
    const [currentPage, setCurrentPage] = useState(1);
    const [allSantriNonAnakAsuhIds, setAllSantriNonAnakAsuhIds] = useState([]);
    ...
    const fetchData = useCallback(async (force = false) => {
        let url = `${API_BASE_URL}data-pokok/santri-non-anakasuh`;
        const params = [];
        ...
        if (filters?.angkatanSantriNonAnakAsuh) params.push(`angkatan_santriNonAnakAsuh=${encodeURIComponent(filters.angkatanSantriNonAnakAsuh)}`);
        ...
        setLoadingSantriNonAnakAsuh(true);
        setError(null);
        ...
        try {
            ...
            console.log("Data dari API SantriNonAnakAsuh:", data);

            setSantriNonAnakAsuh(Array.isArray(data.data) ? data.data : []);
            setTotalDataSantriNonAnakAsuh(data.total_data || 0);
            setTotalPages(data.total_pages || 1);
            // setCurrentPage(data.current_page || 1);
        } catch (err) {
            console.error("Fetch error:", err);
            setError(err.message);
            setSantriNonAnakAsuh([]);
        } finally {
            setLoadingSantriNonAnakAsuh(false);
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentPage, filters, limit, debouncedSearchTerm]);

    const fetchAllData = useCallback(async () => {
        let url = `${API_BASE_URL}data-pokok/santri-non-anakasuh`; // pastikan endpoint ini hanya kembalikan ID saja
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error("Gagal fetch semua ID santriNonAnakAsuh");
            }
            ...
            if (Array.isArray(result.data)) {
                ...
                console.log("Semua ID SantriNonAnakAsuh:", ids);
                // Optional: simpan ke state kalau kamu butuh
                setAllSantriNonAnakAsuhIds(ids);
            } else {
                setAllSantriNonAnakAsuhIds([]);
            }
        } catch (err) {
...
export default useFetchSantriNonAnakAsuh;

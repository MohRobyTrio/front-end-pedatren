...
const useFetchPeserta = (filters) => {
    ...
    const lastRequest = useRef(""); // Menyimpan request terakhir
    const debounceTimeout = useRef(null); // Timeout untuk debounce
    ...
    const fetchData = useCallback(async () => {
        ...
        // Cegah API dipanggil berulang dengan request yang sama
        if (lastRequest.current === url) {
            console.log("Skipping duplicate API call:", url);
            return;
        }
    ...
    useEffect(() => {
        // Gunakan debounce untuk mengurangi jumlah API calls
        if (debounceTimeout.current) {
            clearTimeout(debounceTimeout.current);
        }
        debounceTimeout.current = setTimeout(() => {
            fetchData();
        }, 500); // Tunggu 500ms sebelum eksekusi API

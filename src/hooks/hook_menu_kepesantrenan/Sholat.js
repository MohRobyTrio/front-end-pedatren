...
const useFetchSholat = () => {
    ...
    const fetchSholat = useCallback(async () => {
        setLoadingSholat(true);
        setError(null);

        try {
            const response = await fetch(`${API_BASE_URL}sholat`, {
                headers: {
            ...
            setSholat(Array.isArray(result) ? result : []);
        } catch (err) {

...
const useFetchDataUserOutlet = () => {
    const { clearAuthData } = useLogout();
    const navigate = useNavigate();
    const [dataUserOutlet, setDataUserOutlet] = useState(() => {
        const stored = sessionStorage.getItem("dataUserOutletData");
        return stored ? JSON.parse(stored) : [];
    });

    const [loadingDataUserOutlet, setLoadingDataUserOutlet] = useState(true);
    const [error, setError] = useState(null);
    const token = sessionStorage.getItem("token") || getCookie("token");

    const fetchDataUserOutlet = useCallback(async () => {
        setLoadingDataUserOutlet(true);
        setError(null);

        try {
            const response = await fetch(`${API_BASE_URL}detail-user-outlet`, {
                headers: {
            ...
            setDataUserOutlet(dataArray);
            sessionStorage.setItem("dataUserOutletData", JSON.stringify(dataArray));
        } catch (err) {
            console.error("Fetch error:", err);
            setError(err.message);
            setDataUserOutlet([]);
        } finally {
            setLoadingDataUserOutlet(false);
        }
    ...
    const handleToggleStatus = async (data) => {
        ...
        try {
            ...
            sessionStorage.removeItem("dataUserOutletData");
            fetchDataUserOutlet();
        } catch (error) {
...
export default useFetchDataUserOutlet;

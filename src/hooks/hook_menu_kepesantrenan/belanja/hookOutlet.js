...
const useFetchDataOutlet = () => {
    const { clearAuthData } = useLogout();
    const navigate = useNavigate();
    const [dataOutlet, setDataOutlet] = useState(() => {
        const stored = sessionStorage.getItem("dataOutletData");
        return stored ? JSON.parse(stored) : [];
    });

    const [loadingDataOutlet, setLoadingDataOutlet] = useState(true);
    const [error, setError] = useState(null);
    const token = sessionStorage.getItem("token") || getCookie("token");

    const fetchDataOutlet = useCallback(async () => {
        setLoadingDataOutlet(true);
        setError(null);
    ...
    const handleToggleStatus = async (data) => {
        ...
        try {
            ...
            sessionStorage.removeItem("dataOutletData");
            fetchDataOutlet();
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            const token = sessionStorage.getItem("token") || getCookie("token");
            const response = await fetch(`${API_BASE_URL}outlet/${id}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
...
export default useFetchDataOutlet;

...
const AlumniSantri = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (isProses) {
            const confirmResultlulus = await Swal.fire({
                title: "Konfirmasi Alumni",
                text: "Santri ini akan diproses sebagai alumni. Jika memiliki kartu RFID, kartu tersebut akan dinonaktifkan. Yakin ingin melanjutkan?",
                icon: "warning",
                showCancelButton: true,
            ...
            if (!confirmResultlulus.isConfirmed) return

        } else {
            const confirmResultbatal = await Swal.fire({
                title: "Batalkan Alumni",
                text: "Status alumni santri ini akan dibatalkan. Jika sebelumnya memiliki kartu RFID, kartu tersebut akan diaktifkan kembali. Yakin ingin melanjutkan?",
                icon: "info",
                showCancelButton: true,
                confirmButtonText: "Ya, lanjutkan",
                cancelButtonText: "Batal",
            ...
            if (!confirmResultbatal.isConfirmed) return;
        }
                                                ...
                                                {loadingLulus ? (
                                                    ...
                                                    dataLulus.map((item, index) => (
                                                        <tr key={item.id} className="hover:bg-orange-50/50 transition-colors duration-200">
                                                            ...
                                                            <td className="px-3 sm:px-6 py-3 sm:py-4 text-center">
                                                                <span
                                                                    className={`inline-flex px-2 sm:px-3 py-1 sm:py-1.5 text-xs font-semibold rounded-full ${item.status == "alumni"

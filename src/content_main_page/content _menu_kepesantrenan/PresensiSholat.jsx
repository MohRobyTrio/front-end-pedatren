...
const PresensiSholat = () => {
    ...
    // performSystemChecks menggunakan hasil checkNFCHardware() untuk mengisi checks.hardware
    const performSystemChecks = async () => {
        ...
        let checks = {
            ...
            permissions: false,
            hardware: false, // akan di-set setelah checkNFCHardware
        };
        ...
        // Cek hardware dan ambil hasilnya (boolean)
        const hardwareAvailable = await checkNFCHardware();
        checks.hardware = !!hardwareAvailable;

        // Sekali set semua checks (gabungkan)
    ...
    // kembalikan boolean dan set nfcHardwareEnabled juga
    const checkNFCHardware = async () => {
        try {
            ...
            try {
                // percobaan singkat scan untuk menguji hardware (bisa memicu NotAllowedError)
                ...
                if (error.name === "NotAllowedError") {
                    ...
                    // hardware physically exists -> return true (karena hardware ada)
                    return true;
                    ...
                    // sering muncul ketika tidak ada user gesture - hardware mungkin ada
                    setNfcHardwareEnabled("unknown");
                    console.log("⚠️ NFC Hardware: InvalidStateError (likely needs user gesture)");
                    return true; // kita anggap hardware ada tetapi perlu user gesture
                } else {
                    setNfcHardwareEnabled("unknown");
                    console.log("⚠️ NFC Hardware: Unknown -", error.message);
                    return false;
                }
            }
        } catch (error) {
